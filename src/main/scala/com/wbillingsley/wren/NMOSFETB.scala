...
/**
 * An n-channel enhancement mode MOSFET showing the internals of the device
 * @param pos
 */
class NMOSFETB(pos:(Int, Int), orientation: Orientation = East) extends Component {
  ...
  override def render: VHtmlDiffNode = {
    ...
    SVG.g(^.cls := "wren-component", ^.attr("transform") := s"translate($x, $y) rotate(${orientation.deg})",
      ...
      SVG.rect(^.attr("x") := (-50), ^.attr("y") := (-20), ^.attr("width") := 100, ^.attr("height") := channelWidth, ^.cls := "source semiconductor ndoped channel"),

      SVG.rect(^.attr("x") := (-100), ^.attr("y") := (-20), ^.attr("width") := 50, ^.attr("height") := 20, ^.cls := "source semiconductor ndoped"),
      SVG.rect(^.attr("x") := (50), ^.attr("y") := (-20), ^.attr("width") := 50, ^.attr("height") := 20, ^.cls := "drain semiconductor ndoped"),
