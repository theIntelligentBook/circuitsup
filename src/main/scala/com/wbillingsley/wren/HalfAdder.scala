...
case class HalfAdder(pos:(Int,Int), orientation:Orientation = East)(implicit colouringRule: Wire.ColouringRule = Wire.defaultColouring) extends Component {
  ...
  val tr = new Terminal((x + 70, y + 20), Some(0))
  val tc = new Terminal((x + 70, y + 40), Some(0))
  ...
  override def render: VHtmlDiffNode = {
    ...
    SVG.g(^.cls := "wren-component half-adder", ^.attr("transform") := s"translate($x, $y) rotate(${orientation.deg})",
      ...
      SVG.path(^.cls := colouringRule(tr.current, tr.potential), ^.attr("d") := "M 60 20 l 10 0"),
      SVG.path(^.cls := colouringRule(tc.current, tc.potential), ^.attr("d") := "M 60 40 l 10 0"),
      ...
      SVG.text(^.cls := "output", ^.attr("x") := "55", ^.attr("y") := "20", "Result"),
      SVG.text(^.cls := "output", ^.attr("x") := "55", ^.attr("y") := "40", "Carry"),
    )
  }
}