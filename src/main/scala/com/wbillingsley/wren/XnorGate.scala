...
class XnorGate(pos:(Int,Int), orientation:Orientation = East)(implicit colouringRule: Wire.ColouringRule = Wire.defaultColouring) extends Component {

  val (x, y) = pos

  val ta = new Terminal((x - 30, y - 10), Some(0))
  val tb = new Terminal((x - 30, y + 10), Some(0))
  val out = new Terminal((x + 45, y), Some(0))

  override def terminals: Seq[Terminal] = Seq(ta, tb, out)

  def inputs = Seq(ta.potential, tb.potential)

  override def constraints: Seq[Constraint] = ta.constraints ++ tb.constraints ++ out.constraints :+
    EquationConstraint("XNOR gate", out.potential, inputs, () => for { v <- value } yield if (v) LogicProbe.vdd else LogicProbe.vss)
  ...
  override def render = {
    ...
    SVG.g(^.cls := "wren-component logic-probe", ^.attr("transform") := s"translate($x, $y) rotate(${orientation.deg})",
      ...
      SVG.path(^.cls := colouringRule(out.current, out.potential), ^.attr("d") := "M 35 0 l 10 0"),
      ...
      SVG.circle(^.attr("cx") := "30", ^.attr("cy") := "0", ^.attr("r") := "5")
    )
  }
}
