...
import com.wbillingsley.veautiful.html.{<, DElement, SVG, VHtmlElement, ^, DSvgContent}
...
case class ScatterPlot(plotWidth:Int, plotHeight:Int, xName:String, yName:String, xLabel:(Double) => String, yLabel:(Double) => String, defaultxMax:Double, defaultyMax:Double) {
  ...
  def plot(data:Seq[(Double, Double)]):DSvgContent = {
    ...
    def xAxis(ticks:Int, name:String):DSvgContent = {
      <("g", ns=DElement.svgNS)(
    ...
    def yAxis(ticks:Int, name:String):DSvgContent = {
