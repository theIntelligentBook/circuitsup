...
import com.wbillingsley.veautiful.html.{<, Markup, SVG, VHtmlContent, ^}
...
object Common {
  ...
  def linkToRoute(r:Route, s:String):VHtmlContent = <.a(
    ^.href := Router.path(r),
    ^.cls := (if (Router.route == r) "nav-link active" else "nav-link"),
    s
  )

  def leftMenu:VHtmlContent = <("nav")(^.cls := "d-none d-md-block bg-light sidebar",
